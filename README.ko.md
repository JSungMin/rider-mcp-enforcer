...
| **[ue-log-analyzer](ue-log-analyzer/README.ko.md)** | 거대 UE/Unity 에디터 로그 파싱·dedup·분류·검색·스칼라 추출 | Node만 (IDE 불필요) |

하나 또는 둘 다 설치:
```bash
/plugin marketplace add JSungMin/rider-mcp-enforcer
/plugin install rider-mcp-enforcer@rider-mcp-enforcer   # 코드 검색
/plugin install ue-log-analyzer@rider-mcp-enforcer      # 로그 분석
```
...
바꿉니다. 전형적 흐름:
1. `/ue-log-analyzer:logs` → 에러/경고와 그 `file:line` 찾기.
...
```bash
# 1) 이 repo를 Claude Code 플러그인 마켓플레이스로 추가하고 설치
/plugin marketplace add JSungMin/rider-mcp-enforcer
/plugin install rider-mcp-enforcer@rider-mcp-enforcer

# 2) 프록시 의존성 설치 (최초 1회)
cd <플러그인경로>/proxy && npm install
#   <플러그인경로> = Claude Code가 플러그인을 클론한 위치. /plugin 매니저에서 확인.

# 3) 설정 — Claude Code 안에서 그냥 실행:
/rider-mcp-enforcer:setup
#   Rider SSE 엔드포인트를 자동 탐지하고, 프로젝트 경로를 물어본 뒤 config를 기록합니다.
```

그다음 `/reload-plugins` (또는 재시작). `rider-search` MCP 서버와 도구들이 보이는지, 그리고
`grep src/**/*.cpp`가 차단되며 안내 메시지가 뜨는지 확인하세요.
...
# 3) 프록시 의존성이 바뀌었으면 재설치
cd <플러그인경로>/proxy && npm install

# 4) 새 훅/명령어/MCP 서버가 적용되도록 리로드
