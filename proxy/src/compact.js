/*
 * rider-mcp-enforcer — output compaction for VCS commands the Rider index can't help with: a raw
 ...
 * symbol/reference summarizer, applied to VCS output. No spawning here — the `proxy/vcs.mjs` wrapper runs
 * the command and feeds the raw stdout in; the test exercises these directly with canned strings
 * (deterministic, no toolchain). (Code search is NOT here: grep reroutes to the Rider tools, which the proxy
 * summarizes itself — there is no raw grep output to compact.)
 */
