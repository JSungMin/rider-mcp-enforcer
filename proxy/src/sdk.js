...
const SUBDIR = "proxy"; // package.json lives at ROOT/SUBDIR for installed plugins
const TAG = "rider-search";
const DATA = process.env.CLAUDE_PLUGIN_DATA;
const ROOT = process.env.CLAUDE_PLUGIN_ROOT;
const HERE = path.dirname(fileURLToPath(import.meta.url)); // proxy/src
...
function anchors() {
  ...
  a.push(path.join(HERE, "..", "package.json")); // proxy/package.json (dev / local)
  return a;
}
...
if (!req) {
  console.error(
    `[${TAG}] FATAL: could not resolve @modelcontextprotocol/sdk. ` +
      `Check that Node can run 'npm install' for this plugin, then reconnect rider-search.`,
  );
  process.exit(1);
}
...
export const { Client } = await load("client/index.js");
export const { SSEClientTransport } = await load("client/sse.js");
