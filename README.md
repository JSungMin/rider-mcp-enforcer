...
| Layer | File | Effect |
| --- | --- | --- |
| **Enforcement hook** | `hooks/block-code-grep.js` | Intercepts Bash `grep`/`rg`/`find -name`/`git grep` **and the built-in Grep tool** over C/C++/C# source, steering Claude to the Rider MCP tools. **Bash: default `warn`** — the command runs but a nudge is injected into the model's context; `RIDER_ENFORCE=block` hard-denies, `=0` disables. **Grep & Glob tools: warn-only, never block** — they're the right fallback on a just-edited/unindexed file (or when Rider is disconnected), so they're only nudged (on an explicit code signal): the Grep nudge embeds the ready-to-use `search_symbol`/`search_text` call, and a code **Glob** is steered to Rider's index-based file search (`find_files_by_name_keyword` / `find_files_by_glob`). `=0` silences them. (Unlike the LSP-backed sibling vs-token-safer, a symbol-hunt Grep is **not** escalated to a block — this Rider build's semantic search is weaker and IDE-dependent, so blocking could route to a worse tool or strand the model.) MCP search and the Read tool still bypass by design. Non-code text (logs, md, json) passes through. |
...
| **Summarizing proxy** | `proxy/` | An MCP server fronting Rider's MCP. Parses the JSON search responses (`{items:[{filePath,startLine,lineText}],more}`) into compact `path:line  text`, capped at `RIDER_MAX_RESULTS`, factors the shared root path out once as `under <prefix>/` ([common-prefix factoring](#configuration-env)), and injects a default `projectPath`. Stops large-codebase result floods from blowing up context. |
...
| `RIDER_COMPACT_RESULTS` | `1` | `0`/`off` disables **common-prefix factoring** — on a UE tree every match repeats the same long root (`G:/Proj/Source/Game/…`); the proxy prints it once as `under <prefix>/` and indents the relative tails (the full path stays recoverable as `<prefix>/<tail>`). |
| `RIDER_TEXT_STEER` | `1` | `0`/`off` disables the **text→symbol steer** — a raw-text search (`search_text`/`search_regex`/`search_in_files_*`) whose query is really a symbol hunt (a `<Type>` arg, `::`, or CamelCase id) gets a one-line nudge toward `search_symbol`. Honest about Rider's ceiling: its C++ symbol index can miss, so text search stays the fallback (no completeness claim). Also handles an **alternation** (`A\|B\|C` of CamelCase/snake identifiers) — a single `search_symbol` can't take a regex, so the steer lists one per branch. |
| `RIDER_USES_STEER` | `1` | `0`/`off` disables the **"where is it USED?" steer** — after a focused `search_symbol` found a definition, a one-line nudge points at `search_text` for the call sites/references (symbol search matches definitions only; references need the indexed string match, which can miss on un-indexed/UE-macro code). |
| `RIDER_EDIT_WARN` | `1` | `0`/`off` disables the **Bash code-edit steer** — a `sed -i` / `awk` in-place / python-write over a C#/C++ file is file surgery that bypasses Rider's edit tools; a warn-only nudge points at `replace_text_in_file` (won't corrupt with a bad regex) / `rename_refactoring` (updates all references) / `reformat_file`. Never blocks. A read-only `sed`/`awk` or a build script is not nagged. |
