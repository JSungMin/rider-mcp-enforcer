...
| `RIDER_TEXT_STEER` | `1` | `0`/`off` disables the **text→symbol steer** — a raw-text search (`search_text`/`search_regex`/`search_in_files_*`) whose query is really a symbol hunt (a `<Type>` arg, `::`, or CamelCase id) gets a one-line nudge toward `search_symbol`. Honest about Rider's ceiling: its C++ symbol index can miss, so text search stays the fallback (no completeness claim). |
